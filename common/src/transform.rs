    ...
    pub fn apply_guidance(
            ...
            )
        {
            ...
            if data.sub_kind == EntitySubKind::Drone {
                turn_max = Angle::from_radians(2.0 * std::f32::consts::PI)
            };
        ...
        if data.kind == EntityKind::Boat && data.sub_kind != EntitySubKind::Submarine {
            ...
            // non-zero velocity.
            delta_velocity = guidance.velocity_target.to_mps().clamp(
                max_speed * Velocity::MAX_REVERSE_SCALE,
                max_speed
                    * (1.0 / 3.0
                        + 2.0 / 3.0 * (1.0 - ticks.to_secs() / data.max_health().to_secs())),
        ...
        if data.sub_kind == EntitySubKind::Helicopter {
            delta_velocity = guidance
                .velocity_target
                .to_mps()
        ...
        let is_aeroplane = if data.sub_kind == EntitySubKind::Aeroplane
        ...
        if data.kind == EntityKind::Boat {
            max_accel = 1.0 / 3.0
                * (1.0 + is_aeroplane)
                * delta_seconds
                * max_speed.clamp(15.0, 49.0)
                * (1.0 / 3.0 + 2.0 / 3.0 * (1.0 - ticks.to_secs() / data.max_health().to_secs()))
        };
        if data.sub_kind == EntitySubKind::Submarine {
            max_accel = 1.0 / 3.0 * delta_seconds * max_speed.clamp(15.0, 49.0)
        };
