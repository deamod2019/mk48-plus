...
use crate::ui::{SkillState, UiEvent, UiStatusPlaying};
use crate::Mk48Game;
...
pub fn ship_controls(props: &ShipControlsProps) -> Html {
    ...
    html! {
        <Section id="controls" name={data.label.clone()} position={props.position} style={props.style.clone()} closable={false}>
            ...
            // Data-driven skill buttons: automatically renders buttons for all skills the entity has
            {skill_buttons(&props.status, &button_style, &button_selected_style, &disabled_style, &ui_event_callback)}
        </Section>
    }
}
...
fn skill_button(
    ...
    let (label, disabled, selecting) = match &state {
        ...
        SkillState::Selecting => ("选择目标...".to_string(), false, true),
        SkillState::Charging(secs) => (format!("充能 {:.1}s", secs), true, false),
...
// Legacy individual button functions have been removed.
// All skill buttons are now rendered dynamically by skill_buttons() above.
